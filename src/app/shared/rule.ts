import { RuleRank } from './card';

/** A single King's Cup rule: a short title and the instructions to read aloud. */
export interface Rule {
  title: string;
  text: string;
}

/** The complete set of rules, one per rule rank. */
export type RuleSet = Record<RuleRank, Rule>;

/** The default rules shipped with the game, used until the player edits them. */
export const DEFAULT_RULES: Readonly<RuleSet> = {
  ...
  jack: {
    ...
    text: 'I have never ever done something. Anyone who did has to drink.',
  },
  queen: {
